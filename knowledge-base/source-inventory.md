...
| Workday to HRDM | Located, field discovery started; separate report paths identified | Confluence: Workday HRDM pipeline page; HRDM metadata output; OperationsHR Missing Beneficiary and Missing Emergency Contacts SOPs | Workday current and trended objects are visible, including `S_WORKDAY.WD_DATAMARTFEED`, `S_WORKDAY.WD_DATAMARTFEED_TRENDED`, `S_WORKDAY.WORKDAY_TRENDED`, and `S_WORKDAY.V_WORKDAY_TRENDED`. Deep metadata did not find obvious beneficiary or emergency-contact columns. The SOPs identify governed Workday reports, but no approved report/RaaS extract contract was located. |
...
| ServiceNow | External governed-source candidate; Snowflake object not found | Confluence replication documentation; HRDM metadata output; EPA `SNOW Data HRSS/Resolved` weekly export | Expected tables `sn_hr_core_case` and `sn_hr_core_task` did not appear in the HRDM deep search. The EPA weekly resolved-case export includes case, service, location, lifecycle, and `SLA breached` fields and supports SNOW/LOAA discovery only. Production scoring still requires complete/open-case coverage, cadence, site-key mapping, service-specific SLA rules, classification, and source-owner approval. |
...
| SharePoint / OperationsHR | Located | OperationsHR site, SOP folders, HR Reports, UKG Pro, ECHO, SNOW, Investigations, Workday Documents, FLO/temporary-schedule/locker/badge artifacts | Best source for SOPs, process definitions, evidence of workflow, and some current tracker/workbook artifacts. It identifies authoritative site Smartsheet workflows for FLO and temporary schedules and current site locker files, but is not automatically the durable analytical source. |
...
| EPA SNOW resolved-case export | External governed-source candidate | `https://chewycomllc.sharepoint.com/sites/EnterprisePeopleAnalytics/Shared Documents/SNOW Data HRSS/Resolved/OBR Previous Week Cases Resolved-2026_07_26.xlsx?web=1` | Weekly resolved-case sample with case/service/location/lifecycle/SLA-breach fields. Supports SNOW and LOAA discovery; it is not approved for scoring and does not establish complete/open-case coverage or an approved SLA denominator. |
| HR Service definitions | Located | `https://chewycomllc.sharepoint.com/sites/EnterprisePeopleAnalytics/Shared Documents/SNOW Data HRSS/HR Service Definitions.xlsx?web=1` | Candidate controlled vocabulary for service-specific mapping; source-owner and effective-date approval remain required. |
...
| Temporary Schedule Adjustments SOP | Located | `https://chewycomllc.sharepoint.com/sites/OperationsHR/Shared Documents/HRA/Temporary Schedule Adjustments - SOP.pdf?web=1` | Identifies the authoritative site Smartsheet tracker plus Workday-document and UKG schedule-group steps. Tracker inventory, access, site key, and scoring definition remain open. |
| Locker Management SOP and site workbooks | Located, site examples only | OperationsHR Locker Management Process SOP; SDF4 and SDF6 Locker Management spreadsheets in `SDF1456HR/Shared Documents/Lockers` | Provides locker number/status/assignment evidence for two current sites. Network coverage and alignment to the exact Day 1 survey measure remain unverified. |
| Missing Beneficiary SOP | Located | `https://chewycomllc.sharepoint.com/sites/OperationsHR/Shared Documents/HRM/Missing Beneficiary - SOP.pdf?web=1` | Identifies a monthly Workday report as the system-of-record workflow. No approved export/API contract was located. |
| Missing Emergency Contacts SOP | Located | `https://chewycomllc.sharepoint.com/sites/OperationsHR/Shared Documents/HRM/Missing Emergency Contacts - SOP.pdf?web=1` | Identifies a Workday report as the system-of-record workflow. No approved export/API contract was located. |
| Badge-management SOPs and Badge Reprint App | Located; not inventory evidence | OperationsHR Badge Reprint Tracking, CCure Badge Management, and New Hire Badge SOPs | Covers replacements, badge identity/access, and new-hire setup. It does not provide badge/reel/lanyard/ink stock, so the inventory requirement remains manual/hybrid. |
...
| TM Experience/Signage Microsoft Forms builder | Located as future data contract | `https://chewycomllc.sharepoint.com/sites/FCOpsLibraryofDocs/Shared Documents/2026 Roadmap/TM Experience Roadmap/Facility Comfort and Housekeeping/FC_Comfort_Wellbeing_One_Stop_Audit_MS_Forms_Builder_v2 (3).xlsx?web=1` | Defines normalized audit-response fields across TM Experience and signage topics. Live deployment, response completeness, access, and production ownership were not verified. |
...
| Workday Missing Beneficiary Report | SOP located and confirms the report workflow; no approved export/RaaS source, fields, owner, cadence contract, or sample was located. |
| Workday Employee Emergency Contact Info report | SOP located and confirms the report workflow; no approved export/RaaS source, fields, owner, tenure logic, cadence contract, or sample was located. |
